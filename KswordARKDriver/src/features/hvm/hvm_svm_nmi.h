...
typedef struct _KSW_SVM_NMI_CAPTURE {
    ...
    /* 1030: leaf return scratch. A near return deliberately preserves hardware NMI blocking. */
    KSW_SVM_U64 SavedRax, ReturnRip, ReturnRsp, ReturnFlags;
} KSW_SVM_NMI_CAPTURE;
...
/* The private NMI gate has no calls and no compiler-generated context saves. */
typedef char KSW_NMI_ASSERT_SCRATCH[(offsetof(KSW_SVM_NMI_CAPTURE, SavedRax) == 0x1030) ? 1 : -1];
...
/* Requires SVM root, GIF=0, IF=0, host GS/CR3/XSTATE, private host stack, supervisor CET=0.
   ...
   Hardware NMI blocking remains set: the eventual guest handler's actual completed IRET clears it.
   The queued NMI must therefore be delivered/handed off before native release. */
