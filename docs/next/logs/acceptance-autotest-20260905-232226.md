...
时间(UTC): 2026-09-06T03:22:26.9865244Z → 2026-09-06T03:23:16.8748815Z
...
| status | OK |  |
| prepare | OK |  |
| prune:before-self-test-0905-224959 | OK | 自动修剪旧检查点以回收磁盘 |
| prune:before-driver-load-0905-2249 | OK | 自动修剪旧检查点以回收磁盘 |
| checkpoint:self-test | OK |  |
| alive:self-test | OK | 虚拟机仍然响应 |
| self-test | OK |  |
| prune:before-resident-0905-225004 | OK | 自动修剪旧检查点以回收磁盘 |
| checkpoint:resident | OK |  |
| alive:resident | OK | 虚拟机仍然响应 |
| resident | OK |  |
| stop | FAIL | 打不开 \\.\KswordARKLog：win32=2
  2 = 设备不存在（驱动没加载）；5 = 拒绝访问（需要管理员） |

### 备注

- stop 返回 （lastStatus=）；后续级别不再执行。
- 原始 stdout: {"kind":"error","reason":"device-open-failed"}

---

BLOCKED 表示缺能力/权限/样本，**不等于通过**；NOT_RUN 表示这一段本次没跑。
原始 JSON: `acceptance-autotest-20260905-232226.json`
