...
主程序“内核” Dock 中的“内核知识”页把 `第二规划.md` 的 71 个 Windows 内核专题组织成可检索、可验证的双语知识与证据入口。每个专题同时具备两条互相补充的链：
...
- R0 handler 位于 `KswordARKDriver/src/features/research/research_topic_ioctl.c`，只读取当前请求上下文和中央注册表，不串行调用其它 handler，也不触发扫描或 mutation。
...
- `KernelDock/KernelKnowledgeCatalog.*`：12 个分类、71 个稳定 topic ID、实现状态、官方参考 URL 和业务页白名单路由。
- `KernelDock/KernelKnowledgeTab.*`：搜索、筛选、导航、Markdown、主题、语言切换和异步证据对话框。
- `languages/zh-CN.json`、`languages/en-US.json`：所有可见标题、摘要、正文和证据 UI 文案；只能定点编辑。
- `shared/driver/KswordArkResearchIoctl.h`：唯一共享 ABI。
- `KswordARKDriver/src/features/research/research_topic_ioctl.c`：R0 现场快照与业务入口映射。
