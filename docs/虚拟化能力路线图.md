...
| HVM 算术单元测试 | ✅ | `tools/hvm_unit_tests`（55 项）已接入 `ci.yml` 的 `source-integrity` |
...
| P4.1 每核 EPT 分叉 | 🟡 | 分叉机制已实现（copy-on-write，等价域 1 页、带差异 3 页），但尚未按处理器绑定：常驻仍是全核共用默认 EPTP。P2 的单处理器限制因此还在 |
...
| P5 收口 | 🟡 | CI 门禁补齐：`hvm_unit_tests` 此前从未自动跑，现已挂进 `source-integrity`（55 项检查）。nested 升 active、VBS 共存策略仍未开始 |
