...
| P2.5 与既有工具对接 | 🟡 | CheatEnginePlugin 已接：R-1 内存通道支持按 PID 定位（CR3 不出内核），小读走 R-1、大读走 R0。`APIMonitor_x64` 是进程内 inline patch，它要的是用 CLOAK 视图隐藏 patch 字节，**依赖 P4.1** 解除单处理器限制 |
