...
以 Intel SDM（见 [虚拟化规范要点](虚拟化规范要点.md)）复核后，下列几项的结论与
初稿相反。写在这里是因为"为什么不做"比"还没做"更需要留证据。

### #VE 不做 —— 这不是取舍，是模型冲突

`#VE` 是向量 20。在我们的模型里被虚拟化的客体**就是运行中的 Windows 本身**，
而 Windows 的 IDT 第 20 项是 `KiVirtualizationException`——它不期待我们制造的
#VE，收到就会 bugcheck。要让它可用必须改客体 IDT，而改 IDT 会被 PatchGuard 抓。

所以 P2.4 的正确形态是**只做 VMFUNC 那一半**，并且在 EPT 叶项上主动设置
suppress-#VE 位当作防御，同时把 `EPT_VIOLATION_VE`（secondary bit 18）恒定为 0
并加 grep 门禁防止回归。#VE 能力照常探测上报，但不启用。
...
- list 的 512 项不能留空：全 0 是非法 EPTP（页遍历级数为 0），未使用的槽要填成
  与当前 EPTP 相同的合法值。
...
| P2.1–P2.3 EPT 分离视图 | ✅ | `hvm_ept_view.c`、`IOCTL_KSWORD_ARK_HVM_VIEW`、`UI/KvmViewDialog.*`；**单处理器拓扑限制未解除** |
| P2.4 `#VE` + VMFUNC | ⬜ | 未开始；依赖多 EPTP，也就是依赖 P4.1 |
| P2.5 与既有工具对接 | ⬜ | 未开始（`APIMonitor_x64`、`CheatEnginePlugin`） |
| P3.1 AMD 能力探测 | ✅ | AMD 上如实报告 SVM/NPT 与固件锁，状态为 `BACKEND_NOT_IMPLEMENTED` |
| P3.2–P3.7 SVM 后端 | ⬜ | 刻意未写，见 P3 小节取舍说明；需要 AMD 测试机 |
| P4.1 每核 EPT 分叉 | ⬜ | 方案已写进 P4 小节，代码未动；这是 P2 实用性的硬瓶颈 |
| P4.2–P4.6 受控执行域 | ⬜ | 未开始；全部依赖 P4.1 |
| 嵌套运行（作为 L1） | ✅ | 见 §二·六；`tools/hvmprobe` 用于确认环境 |
| P5 收口 | ⬜ | 未开始（nested 升 active、VBS 共存策略、CI 门禁补齐） |
