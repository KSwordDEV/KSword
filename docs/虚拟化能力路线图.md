...
| P1.2 | **MSR 策略引擎**：基于 P0.2 的位图按需开洞；每条策略支持 `记录 / 拒绝 / 伪造读值 / 吞写` | ⬜ 未开始 |
| P1.3 | **CR/DR 策略**：CR0/CR4 影子寄存器、`CR3-load exiting` + CR3 target list（用于进程切换观测）、MOV-DR 拦截 | ⬜ 未开始 |
| P1.4 | **事件 ring v2**：每 CPU 分片 + 多消费者游标 + 丢弃计数（复用 Callback Monitor v1 的 ring 设计与发布协议） | ⬜ 未开始 |
| P1.5 | **R3/UI**：`KernelHvmTab` 拆出策略子页——规则表（含模式列）、实时事件流、速率/丢弃指标 | ⬜ 未开始 |
...
| 编号 | 内容 |
|---|---|
| P2.1 | **EPT 分离视图**：优先走 execute-only EPT（`KSW_EPT_CAP_EXECUTE_ONLY`）；不支持时退化为 MTF 交替视图 |
| P2.2 | **影子页 hook**：目标 GPA 的 exec 访问重定向到影子页（含 hook 指令），read/write 落回原页——对内存扫描不可见的断点 |
| P2.3 | **内存隐藏**：指定 GPA 范围对外呈现零页或伪造内容，读取走影子、执行走原页 |
| P2.4 | **高性能路径**（能力可选）：`#VE` + `VMFUNC` EPTP switching，把视图切换从 VM-exit 降到 guest 内完成 |
| P2.5 | **R3/UI**：Hook 管理页 + 隐藏区域表；与 `APIMonitor_x64`、`CheatEnginePlugin` 的既有接口对接 |
