...
function CheckNestedProbe($Metrics, $Previous) {
    if ($Metrics.version -ne 6 -or $Metrics.backend -ne 2 -or @($Metrics.svmProcessors).Count -ne $Vcpu) {
        throw 'Nested probe requires metrics v6 and a complete AMD CPU set.'
    }
    $seen=@{}
...
function CheckGeneral($Metrics, [bool]$Active) {
    if ($Metrics.version -ne 6 -or $Metrics.backend -ne 2 -or @($Metrics.svmProcessors).Count -ne $Vcpu) {
        throw 'General residency requires metrics v6 and the complete AMD CPU set.'
    }
    $seen=@{}
    foreach ($cpu in $Metrics.svmProcessors) {
        $id="$($cpu.group):$($cpu.number)"
