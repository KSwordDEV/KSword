...
int main(void)
{
    ...
    KswSvmWrite64(&vmcb, KSW_VMCB_EXITCODE, 0x64);
    CHECK(KswSvmNestedIrqWindowRestore(&vmcb, &window, &exit) && exit);
    CHECK((KswSvmRead64(&vmcb, KSW_VMCB_INTCTL) & 256) != 0);
    KswSvmWrite64(&vmcb, KSW_VMCB_INTCTL, (0x21ULL << 32) | 0xe0100ULL);
    KswSvmWrite64(&vmcb, KSW_VMCB_RFLAGS, 0x202);
    CHECK(KswSvmNestedPreferVirq(&vmcb, 0));
    CHECK(!KswSvmNestedPreferVirq(&vmcb, 0x80000031ULL));
    CHECK(!KswSvmNestedPreferVirq(&vmcb, 0x80000202ULL));
    KswSvmWrite64(&vmcb, KSW_VMCB_INTCTL, (0x21ULL << 32) | 0xe010fULL);
    CHECK(!KswSvmNestedPreferVirq(&vmcb, 0));
    KswSvmWrite64(&vmcb, KSW_VMCB_INTCTL, (0x21ULL << 32) | 0x1e010fULL);
    CHECK(KswSvmNestedPreferVirq(&vmcb, 0));
    CHECK(KswSvmNestedIrqWindowArm(&vmcb, 0x80000091ULL, 46, &window));
    CHECK((KswSvmRead64(&vmcb, KSW_VMCB_INTCTL) & 0x1f0000ULL) == 0x1e0000ULL);
    CHECK(KswSvmNestedIrqWindowRestore(&vmcb, &window, &exit));
    CHECK(KswSvmRead64(&vmcb, KSW_VMCB_INTCTL) == ((0x21ULL << 32) | 0x1e010fULL));
    KswSvmWrite64(&vmcb, KSW_VMCB_EVENT, exception);
    CHECK(!KswSvmNestedPreferVirq(&vmcb, 0));
    KswSvmWrite64(&vmcb, KSW_VMCB_EVENT, 0);
    KswSvmWrite64(&vmcb, KSW_VMCB_RFLAGS, 2);
    CHECK(!KswSvmNestedPreferVirq(&vmcb, 0));
